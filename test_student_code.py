        ...
        if fact_rule.asserted:
            if fact_rule.supported_by:
                for pair in fact_rule.supported_by:
                    if pair[0] not in self.rules or pair[1] not in self.facts:
                        fact_rule.supported_by.remove(pair)
                if fact_rule.supported_by:
                ...
                for supp_fact in fact_rule.supp_facts:
                    if len(supp_fact.supported_by) == 1:
                        self.kb_retract(supp_fact)
                    elif len(supp_fact.supported_by) > 1:
                        for pair in supp_fact.supported_by:
                            if fact_rule in pair:
                                supp_fact.supported_by.remove(pair)
